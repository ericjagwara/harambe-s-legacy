...
export default function Registration({ initialTab = 'run' }: { initialTab?: RegistrationTab }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>, label: string) => {
    ...
    const details = Array.from(data.entries())
      ...
      .join('\n')
    const subject = encodeURIComponent(`Harambe Run 2026, ${label}`)
      ...
      if (status === 'SUCCESSFUL') {
        ...
        setSubmitted(
          purpose === 'Donation'
            ...
            : `Payment received. Your place at Harambe Run 2026 is confirmed. We will send your kit collection details and start point briefing by email. Harambe. Run. Fund. Job Creation. Tell a friend!`
        )
                ...
                >
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <div className="flex items-end">
                      <p className="border border-border bg-background p-4 text-sm leading-6 text-foreground/70">
                        No ID upload needed now. Bring your student ID with you at kit pickup to confirm the student rate.
                      </p>
                    </div>
                  </div>
